...
if ( ! class_exists( 'typit_bts_about_page' ) ) {
	/**
	 * Singleton class used for generating the about page of the theme.
	 */
	class typit_bts_about_page {
		/**
		 * Define the version of the class.
		 *
		 * @var string $version The typit_bts_about_page class version.
		 */
		...
		/**
		 * The single instance of typit_bts_about_page
		 *
		 * @var typit_bts_about_page $instance The  typit_bts_about_page instance.
		 */
		private static $instance;

		/**
		 * The Main typit_bts_about_page instance.
		 *
		 * We make sure that only one instance of typit_bts_about_page exists in the memory at one time.
		 *
		 * @param array $config The configuration array.
		 */
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof typit_bts_about_page ) ) {
				self::$instance = new typit_bts_about_page;
		...
		function register() {
			if ( ! empty( $this->menu_name ) && ! empty( $this->page_name ) ) {
				add_theme_page( $this->menu_name, $this->page_name, 'activate_plugins', 'bts-welcome', array(
					$this,
					'page_render',
				) );
			}
			...
			if ( ! empty( $welcome_title ) || ! empty( $welcome_content ) || ! empty( $this->tabs ) ) {
				...
				if ( ! empty( $welcome_title ) ) {
					...
					if ( ! empty( $this->theme_version ) ) {
						echo '<small class="about-version">' . esc_html__(' - v', 'typit'), esc_html( $this->theme_version ) . ' </small>';
					}
					echo '</h1>';
				}
				...
				if ( ! empty( $this->tabs ) ) {
					...
					foreach ( $this->tabs as $tab_key => $tab_name ) {
						...
						echo '<a href="' . esc_url( admin_url( 'themes.php?page=bts-welcome' ) ) . '&tab=' . esc_attr($tab_key) . '" class="nav-tab ' . ( $active_tab == $tab_key ? 'nav-tab-active' : '' ) . '" role="tab" data-toggle="tab">';
		...
		public function getting_started() {
			...
			if ( ! empty( $this->config['getting_started'] ) ) {
				...
				if ( ! empty( $getting_started ) ) {
					...
					echo '<div id="bts-getting-started" class="feature-section three-col">';
		...
		public function support_content() {
			echo '<div id="bts-support-content" class="feature-section three-col">';
		...
		public function free_pro() {
			...
			if ( ! empty( $free_pro ) ) {
				if ( ! empty( $free_pro['free_theme_name'] ) && ! empty( $free_pro['pro_theme_name'] ) && ! empty( $free_pro['features'] ) && is_array( $free_pro['features'] ) ) {
					echo '<div id="bts-free-pro" class="feature-section">';
					...
					foreach ( $free_pro['features'] as $feature ) {
						...
						echo '<tr' . esc_attr($feature['hidden']) . '>';
					...
					if ( ! empty( $free_pro['pro_theme_link'] ) && ! empty( $free_pro['get_pro_theme_label'] ) ) {
						echo '<tr class="bts-free-pro-button">';
						echo '<td></td>';
		...
		public function style_and_scripts( $hook_suffix ) {
			...
			// enqueue css files
			if ( 'themes.php' === $pagenow || 'appearance_page_bts-welcome' == $hook_suffix ) {
				wp_enqueue_style( 'typit-bts-about-page-css', get_template_directory_uri() . '/inc/about/css/typit-info-backend.css' );
			}

			// enqueue js files
			if ( 'appearance_page_bts-welcome' == $hook_suffix ) {
				wp_enqueue_script( 'typit-bts-about-page-js', ( get_template_directory_uri() . '/inc/about/js/typit-info-backend.js' ), array( 'jquery' ), '', 'true' );
			}
