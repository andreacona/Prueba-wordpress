...
// Text Area
 function typit_sanitize_textarea($input){
	return wp_kses_post( $input );
}

// Strip Slashes
	function typit_sanitize_strip_slashes($input) {
		return wp_kses_stripslashes($input);
	}	

// Sanitize range slider
function typit_sanitize_range( $input ) {
	filter_var( $input, FILTER_FLAG_ALLOW_FRACTION );
	return ( $input );
}

// Adds sanitization callback function: Slider Category
function typit_sanitize_slidecat( $input ) {
	if ( array_key_exists( $input, typit_slide_cats() ) ) {
		return $input;
	} else {
		return '';
	}
 ...
 */
function typit_sanitize_checkbox( $checked ) {
 ...
 */
function typit_sanitize_select( $input, $setting ) {
